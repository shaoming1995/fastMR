...
#' @param triat 输入表型的名称
#' @export
GWAS_meta<-function(data1,data2,snp_col="SNP",beta_col="BETA",se_col="SE",filename1="芬兰肺癌GWAS",filename2="catlog肺癌GWAS",Nsample=1000000,triat="Lung cancer"){
  ...
  message("正在获取有效SNP...")
  d1_d2<-merge(data1,
               data2[,c(snp_col,beta_col,se_col)],
               by.x = "SNP",
               by.y = snp_col,
               all = F)
  #查看有效SNP数目
  ...
  d22<-d1_d2[,c("SNP",beta_col,se_col,"X.chrom","pos","effect_allele.exposure","other_allele.exposure","pval.exposure")]

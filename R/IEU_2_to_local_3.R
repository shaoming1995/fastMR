...
#' @export
IEU_2_to_local_3<-function(keyssh,data1IV,GWASID1,data2IV,GWASID2,data3IV,data3GWAS,data4IV,data4GWAS,data5IV,data5GWAS){
  ...
  if (Sys.info()["nodename"] == keyssh) {
    ...
    data_h_SNP_steiger_mv5_1<-extract_outcome_data(snps=data5IV$SNP,outcomes=GWASID1,proxies=T,maf_threshold = 0.01,access_token = NULL)
    data_h_SNP_steiger_mv5_1<-data_h_SNP_steiger_mv5_1[,out_name]
    colnames(data_h_SNP_steiger_mv5_1)<-exp_name
    data_h_SNP_steiger_mv5_2<-extract_outcome_data(snps=data5IV$SNP,outcomes=GWASID2,proxies=T,maf_threshold = 0.01,access_token = NULL)
